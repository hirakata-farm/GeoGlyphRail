...
L.Handler.MarkerSnap = L.Handler.extend({
    options: {
        snapDistance: 30, // in pixels default 15 in original
        snapVertices: false // default true in original
    },
...
//    },
    addGuideLayer: function (layer) {
        for (var i=0, n=layer.length; i<n; i++) this._guides.push(layer[i]);
    },
...
//        }
	...
	if ( closest ) {
            if (e.originalEvent && e.originalEvent.clientX && closest.latlng) {
		var snapTouchPoint = this._map.project(closest.latlng, this._map.getZoom());
		e.originalEvent.clientX = snapTouchPoint.x;
		e.originalEvent.clientY = snapTouchPoint.y;
		e.originalEvent.snapped = true;
