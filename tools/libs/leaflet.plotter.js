L.Polyline.plotter = L.Polyline.extend({
    ...
    _unbindMarkerEvents: function(marker){
        marker.off('dblclick', this._removePoint, this);
        marker.off('drag', this._replot, this);
        marker.dragging.disable()
    },
    _bindMarkerEvents: function(marker){
        marker.on('dblclick', this._removePoint, this);
	...
	var num = this._lineMarkers.length;
        var newMarker = this._getNewMarker(e.latlng, { icon: this._editIcon , title : num , alt : num });
    ...
    _redrawHalfwayPoints: function(){
        ...
        for(index in this._lineMarkers){
            ...
            ], { icon: this._editIcon, opacity: 0.3 }).addTo(this._map);
    ...
    _plotExisting: function(){
        for(index in this._existingLatLngs){
            this._addNewMarker({
                latlng: new L.LatLng(
                    this._existingLatLngs[index].lat,
                    this._existingLatLngs[index].lng
                )
            });
        }
	this._replot();
